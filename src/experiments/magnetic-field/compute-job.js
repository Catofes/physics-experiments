...
export function computeHeatData(field, section, size) {
  const S = HEAT_RES,
    ...
    pixels = new Uint8ClampedArray(S * S * 4);
  ...
  for (let row = 0; row < S; row++)
    for (let col = 0; col < S; col++) {
      point
        .copy(section.center)
        .addScaledVector(section.U, (col / S - 0.5) * size)
        .addScaledVector(section.V, (0.5 - row / S) * size);
      const i = row * S + col;
      if (field.heatMask?.(point.x, point.y, point.z)) {
        values[i] = NaN;
        continue;
      }
...
export function computeJob({ sceneId, params, section, size, kinds }) {
  const start = performance.now(),
    field = buildField(sceneId, params),
    result = {};
  ...
  if (kinds.includes("heat"))
    result.heat = computeHeatData(field, computeSectionBasis(section), size);
  if (kinds.includes("section"))
    result.section = {
      lines: pack(
        generateProjectedStreamlines(
          field.evalB,
          computeSectionBasis(section),
          size,
          field,
        ),
      ),
...
export function transferables(result) {
  return [
    ...
    ...(result.heat ? [result.heat.pixels] : []),
